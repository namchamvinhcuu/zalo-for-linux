...
// stderr carries Wine's own diagnostics (page faults etc.); route to ZCALL_WINE_LOG if
// set so it doesn't interleave with the event stream consumers, else inherit.
const wineErr = process.env.ZCALL_WINE_LOG ? fs.openSync(process.env.ZCALL_WINE_LOG, 'a') : 'inherit';
const shim = spawn(WINE, [SHIM_EXE, PIPE_RECV, PIPE_SEND], {
  cwd: ENGINE_DIR,
  env: { ...process.env, WINEPREFIX, WINEDEBUG: process.env.WINEDEBUG || '-all' },
...
function ack() { try { sendSock.write(Buffer.from([0x06])); } catch (_) {} }
...
process.on('SIGINT', () => { try { shim.kill('SIGINT'); } catch (_) {} process.exit(0); });
process.on('SIGTERM', () => { try { shim.kill('SIGTERM'); } catch (_) {} process.exit(0); });
